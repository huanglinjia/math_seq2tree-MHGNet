...
for fold in range(5):
    ...
    for epoch in range(n_epochs):
        ...
        for idx in range(len(input_lengths)):
            graph_batch = get_single_graph_batch(graph_info_batches[idx], input_batches[idx], num_pos_batches[idx],
                                                 unk_pos_batches[idx])
            loss = train_tree(
                input_batches[idx], graph_batch, input_lengths[idx], output_batches[idx], output_lengths[idx],
        ...
        if epoch % 10 == 0 or epoch > n_epochs - 5:
            ...
            #
            #     graph_batch = get_single_graph_batch(graph_info_batches_test[idx], input_batches_test[idx],
            #                                          num_pos_batches_test[idx],
            #                                          unk_pos_batches_test[idx])
            #
            #     test_res = evaluate_tree(test_batch[0], graph_batch, test_batch[1], generate_num_ids, encoder, predict, generate,
            ...
            for idx in tqdm(range(len((input_lengths_test)))):
                ...
                out, loss = evaluate_tree(input_batches_test[idx], graph_batch, input_lengths_test[idx], generate_num_ids, encoder, predict, generate, \
                                          merge, output_lang,  num_pos_batches_test[idx], beam_size=beam_size)
