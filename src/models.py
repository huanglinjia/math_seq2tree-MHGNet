...
class EncoderSeq(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        ...
        self.gcn = Graph_Module(hid_dim, hid_dim, hid_dim)

    def forward(self, input_seqs, batch_graph, input_lengths, hidden=None):
        # Note: we run this all at once (over multiple batches of multiple sequences)
        embedded = self.embedding(input_seqs)  # S x B x E
        embedded = self.em_dropout(embedded)
        _, embedded = self.gcn(embedded.transpose(0,1), batch_graph)
...
class Graph_Module(nn.Module):
    ...
    def forward(self, graph_nodes, graph):
        ...
        if not bool(graph.numel()):
            ...
            adj = graph.float()
            adj_list = [adj[:, 0, :], adj[:, 0, :], adj[:, 1, :], adj[:, 2, :]]
        # print(adj)
