...
class Lang:
    ...
    def add_sen_to_vocab(self, sentence, word_sentence):  # add words of sentence to vocab
    ...
    def add_sen_to_vocab_tree(self, sentence):  # add words of sentence to vocab
        for word in sentence:
            if re.search("N\d+|NUM|\d+", word):
                continue
            if word not in self.index2word:
    ...
    def trim(self, min_count):  # trim words below a certain count threshold
        ...
        for word in keep_words:
            ...
            if self.is_chinese(word) and len(word) > 1:
                word_lists = list(word)
                t = len(word_lists)
                for i in range(t - 2):
                    c = []
                    for j in range(len(word_lists) - 1):
                        c.append(word_lists[j] + word_lists[j + 1][-1])
                    word_lists = c
                    for w in word_lists:
...
def transfer_num(data):  # transfer num into "NUM"
    ...
    for d in data:
        ...
        pairs.append((input_seq, out_seq, nums, num_pos, word_seq))
...
def indexes_from_sentence_inputlang(lang, word_sentence, tree=False):
    res = []
    sentence_pos = []
    idx = 0
    for word in word_sentence:
        if len(word) == 0:
            continue
        if word == "NUM":
            sentence_pos.append(idx)
        if word in lang.word2index:
            if not lang.is_chinese(word):
                res.append(lang.word2index[word])
                idx += 1
                continue
            tail = word
            while len(tail) >= 1:
                pos = np.arange(len(tail)) + 1
                p = np.random.choice(pos)
                head, tail = tail[:p], tail[p:]
                res.append(lang.word2index[head])
                idx += 1
...
def prepare_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, \
          input_lang=None, output_lang=None, tree=False):
    train_pairs = []
    test_pairs = []
    ...
    if input_lang==None and output_lang==None:
      ...
      for pair in pairs_trained:
          if not tree:
              input_lang.add_sen_to_vocab(pair[0], pair[4])
              output_lang.add_sen_to_vocab_tree(pair[1])
          elif pair[-1]:
              input_lang.add_sen_to_vocab(pair[0], pair[4])
              output_lang.add_sen_to_vocab_tree(pair[1])
    ...
    for pair in pairs_trained:
        ...
        input_cell, sentence_pos = indexes_from_sentence_inputlang(input_lang, pair[4])
        input_cell_1 = indexes_from_sentence(input_lang, pair[0])

        # train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
        #                     pair[2], pair[3], num_stack, pair[4]))

        train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
                          pair[2], sentence_pos, num_stack))
        if len(input_cell) != len(input_cell_1):
          output_cell_1 = copy.deepcopy(output_cell)
          n1 = copy.deepcopy(pair[2])
          n2 = copy.deepcopy(num_stack)
          train_pairs.append((input_cell_1, len(input_cell_1), output_cell_1, len(output_cell_1),
                            n1, pair[3], n2))
          del output_cell_1, n1, n2
...
def prepare_test(pairs, input_lang, output_lang, tree=True):
    train_pairs = []
